...
- Available transfer primitive: `net.openfederation.community.transfer` (owner-only) returns an export package plus a random token accompanied by a 24-hour expiry timestamp. The token is not currently signed, persisted for one-time consumption, or verified by `admin.importRepo`.
