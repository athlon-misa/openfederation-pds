...
describe('AES-256-GCM key encryption', () => {
  it('encrypt then decrypt roundtrips correctly', () => {
    ...
    const encrypted = encryptKeyBytes(original);
    const decrypted = decryptKeyBytes(encrypted);
    assert.ok(original.equals(decrypted), 'Decrypted should equal original');
  });

  it('encrypted output is larger than input (salt + iv + authTag)', () => {
    const original = crypto.randomBytes(32);
    const encrypted = encryptKeyBytes(original);
  ...
  it('produces different ciphertexts for the same input (random IV/salt)', () => {
    const original = crypto.randomBytes(32);
    const encrypted1 = encryptKeyBytes(original);
    const encrypted2 = encryptKeyBytes(original);
  ...
  it('fails to decrypt with truncated data', () => {
    const original = crypto.randomBytes(32);
    const encrypted = encryptKeyBytes(original);
