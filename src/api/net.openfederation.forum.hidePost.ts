...
import { setPostHidden } from '../forum/forum-index.js';
import { query } from '../db/client.js';
import { auditLog } from '../db/audit.js';

export default async function hidePost(req: AuthRequest, res: Response): Promise<void> {
  try {
    ...
    const lookup = await query<{ community_did: string }>(
      'SELECT community_did FROM forum_posts WHERE uri = $1', [uri]
    );
    ...
    await setPostHidden(uri, hidden);
    await auditLog(hidden ? 'forum.post.hide' : 'forum.post.unhide', req.auth!.userId, communityDid, { uri });
    res.status(200).json({ success: true });
  } catch (error) {
    console.error('Error in forum.hidePost:', error);
    res.status(500).json({ error: 'InternalServerError', message: 'Failed to update post visibility' });
  }
}
