...
export default async function approveAccount(req: AuthRequest, res: Response): Promise<void> {
  if (!requireRole(req, res, ['admin', 'moderator'])) {
    return;
  }

  const input: ApproveInput = req.body || {};
  ...
  const values: string[] = [req.auth!.userId];
  let paramIndex = 2;
  ...
  await auditLog('account.approve', req.auth!.userId, rows[0].id, {
