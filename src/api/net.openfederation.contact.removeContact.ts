...
import { removeContact } from '../contact/index.js';

const NSID = 'net.openfederation.contact.removeContact';

export default async function removeContactHandler(req: AuthRequest, res: Response): Promise<void> {
  try {
    ...
    await removeContact(req.auth!, subject).catch(err => {
